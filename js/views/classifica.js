...
export function renderTrendMobile(posPrec, posCorrente) {
  ...
  const delta = posPrec - posCorrente;
...
function setLeaderboardHtml(html) {
  const el = document.getElementById('leaderboard'); if (el) el.innerHTML = html;
  const el2 = document.getElementById('leaderboardTab'); if (el2) el2.innerHTML = html;
}
...
async function getMyLbHash() {
  ...
  try {
    const email = (auth.currentUser && auth.currentUser.email || '').toLowerCase().trim();
...
// Il doc leaderboard è aggregato dalla Cloud Function una volta l'ora:
// cache client con TTL per non rifare il getDoc a ogni cambio tab.
const LB_TTL_MS = 5 * 60 * 1000;
async function fetchLeaderboardDoc(mese) {
  if (S.lbCache && S.lbCache.mese === mese && (Date.now() - S.lbCache.ts) < LB_TTL_MS) {
    return S.lbCache.data;
  }
  const snap = await getDoc(doc(db, 'leaderboard', mese));
  const data = snap.exists() ? snap.data() : null;
  S.lbCache = { mese: mese, ts: Date.now(), data: data };
  return data;
}

export async function loadLeaderboard() {
  const meseCorrente = meseCorrenteRoma();
  const myHash = await getMyLbHash();
  const myName = (S.dp.cognome || '').toUpperCase().trim();

  try {
    const lb = await fetchLeaderboardDoc(meseCorrente);
    if (!lb || ((lb.drivers || []).length === 0)) {
      ...
      const hasMyReports = S.reports.some(function (r) {
        return recordYMD(r).substring(0, 7) === meseCorrente;
      });
      const msg = hasMyReports
    ...
    const sorted = lb.drivers || [];
    const totalDrivers = lb.totalDrivers || sorted.length;
    ...
    top10.forEach(function (entry, i) {
      ...
      const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : String(pos);
      const nick = getNickname(entry.h || entry.driver || '');
      const youTag = isMe ? '<span class="lb-you">SEI TU!</span>' : '';
      const bonusTag = (entry.bonusVelocita > 0 ? '<span class="lb-bonus" title="Bonus velocità">⚡</span>' : '') + (entry.bonusZeroDanni ? '<span class="lb-bonus">🛡️</span>' : '');
      const trend = renderTrendMobile(entry.posPrec, pos);
    ...
    if (myPos >= 10) {
      const myData = sorted[myPos];
      const myBonus = (myData.bonusVelocita > 0 ? '<span class="lb-bonus">⚡</span>' : '') + (myData.bonusZeroDanni ? '<span class="lb-bonus">🛡️</span>' : '');
      const myTrend = renderTrendMobile(myData.posPrec, myPos + 1);
      // Percentile: 1° = 100%, ultimo = ~ (1/total)*100
      const percentile = Math.max(1, Math.round((1 - myPos / totalDrivers) * 100));
